...
const generateWords = (
        ...
        if (word.length >= 4) {
            if (potentialWords.includes(word) && !currWords.includes(word)) {
                ...
                generateWords(lastLetter, boxData, efficiency, '', currWords, dictionary, starttime)

                // If we get here, we either have a complete list, or that word didn't work out
        ...
        for (const nextLetter of boxData[letter]) {
            ...
            word += nextLetter
            generateWords(nextLetter, boxData, efficiency, word, currWords, potentialWords, starttime)
            word = word.slice(0, -1)
        }
